...
plot_save_dir <- here("SDR001_dCas9_Plots")
path_dir_gDNA_reads <- here("SDRranger_gDNA_dCas9/raw_reads_bc_matrix")
path_dir_cDNA_reads <- here("SDRranger_RNA_dCas9/raw_reads_bc_matrix")
path_dir_cDNA_umis <- here("SDRranger_RNA_dCas9/raw_umis_bc_matrix")
panel_size_gDNA = 33
panel_size_RNA  = 47
...
Types <- data.frame(Type = c("dCas9_1", "dCas9_2", "dCas9_3", "dCas9_4", "PE_1", "PE_2", "PE_3", "PE_4"),
...
#Assign cells to gRNAs via CROP-seq transcripts
...
#Only select the CROP entries -  48-108
cDNA_mat_reads_CROP <- cDNA_mat_reads[48:108,]
...
#Only select the CROP entries - first 48-108
cDNA_mat_CROP <- cDNA_mat[48:108,]


#Calculate CROP ratio and total reads

row_names_highest <- apply(cDNA_mat_reads_CROP , 2, function(col) rownames(cDNA_mat_reads_CROP )[which.max(col)])
max_values <- apply(cDNA_mat_reads_CROP , 2, max)
column_sums <- colSums(cDNA_mat_reads_CROP)
ratio_CROP <- max_values/column_sums

#Get highest number of UMIs in cell per CROP construct
max_values_umi <- apply(cDNA_mat_CROP, 2, max)


cDNA_mat_CROP_meta <- data.frame(CROP_assigned = row_names_highest, 
                                       CROP_max_reads = max_values, 
                                       CROP_ratio = ratio_CROP, 
                                       CROP_max_umis = max_values_umi, 
                                       CROP_n_reads_sum = column_sums,
                                       cell_index = colnames(cDNA_mat_CROP))
                                      
#Split cell and sbc

cDNA_mat_CROP_meta  <- cDNA_mat_CROP_meta  %>%
  tidyr::separate(cell_index, into = c("cell", "sbc"), sep = ":", remove = FALSE)
...
cDNA_mat_reads_meta <- cDNA_mat_reads_meta %>% left_join(cDNA_mat_CROP_meta, by = c("cell_index" = "cell_index", "cell" = "cell", "sbc" = "sbc"))
...
#Only select the cDNA entries containing the RNA information - first 1-47
cDNA_mat <- cDNA_mat[1:47,]

#Make Seurat
cDNA_data_Seurat <- CreateSeuratObject(cDNA_mat, project = "SDR001_dCas9")
...
#Make Seurat
cDNA_data_Seurat <- CreateSeuratObject(cDNA_mat_mod, project = "SDR001_dCas9")
...
cDNA_data_Seurat <- subset(x = cDNA_data_Seurat, subset = Type == "dCas9_1" |
                                                          Type == "dCas9_2" |
                                                          Type == "dCas9_3" |
                                                          Type == "dCas9_4" )

saveRDS(cDNA_data_Seurat, paste0(plot_save_dir,"/SDR001_dCas9_Seurat_max_sbc_1000reads_cut.rds"))
...
saveRDS(cDNA_data_Seurat, paste0(plot_save_dir,"/SDR001_dCas9_Seurat_max_sbc_1000reads_cut.rds"))
cDNA_data_Seurat <- readRDS(paste0(plot_save_dir,"/SDR001_dCas9_Seurat_max_sbc_1000reads_cut.rds"))
...
group_colors <- c(dCas9_1 = "#EADFB4", dCas9_2 = "#9BB0C1", dCas9_3 = "#51829B" , dCas9_4 = "#F6995C", 
                  PE_1 = "#EFBC9B", PE_2 = "#FBF3D5", PE_3 = "#D6DAC8", PE_4 = "#9CAFAA")
...
#Plot ratios of CROP transcripts


```{r}
title = "CROP_ratio.pdf"


p <- metadata %>%
 ggplot(aes(x=Type, y=CROP_ratio, fill = Type))+
  geom_violin(alpha = 0.2)+
  geom_boxplot(width = 0.25)+
  	theme_classic() +
  	theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
    ...
    scale_y_continuous(limits = c(0,1))+
  	ggtitle(title)+
      labs(x = "Type", y = "% of maximum CROP transcript per cell")+
    scale_fill_manual(values = group_colors)+
    guides(fill = "none")
...
cDNA_data_Seurat <- subset(x = cDNA_data_Seurat, subset = nCount_RNA >= 600)
cDNA_data_Seurat <- subset(x = cDNA_data_Seurat, subset = nCount_RNA <= 9000)
cDNA_data_Seurat <- subset(x = cDNA_data_Seurat, subset = cDNA_ratio_sbc >= 0.75)


metadata <- cDNA_data_Seurat@meta.data


saveRDS(cDNA_data_Seurat, paste0(plot_save_dir,"/SDR001_dCas9_Seurat_filtered.rds"))

#cDNA_data_Seurat <- subset(x = cDNA_data_Seurat, subset = nCount_RNA >= 100)


cDNA_data_Seurat <- readRDS(paste0(plot_save_dir,"/SDR001_dCas9_Seurat_filtered.rds"))
...
saveRDS(cDNA_data_Seurat, paste0(plot_save_dir,"/SDR001_dCas9_Seurat_filtered_clustered.rds"))

cDNA_data_Seurat <- readRDS(paste0(plot_save_dir,"/SDR001_dCas9_Seurat_filtered_clustered.rds"))
...
title = "SDR001_dCas9_UMAP_Type.pdf"
...
title = "SDR001_dCas9_UMAP_Cluster.pdf"
...
title = "SDR001_dCas9_UMAP_nFeature_gDNA.pdf"
...
title = "SDR001_dCas9_UMAP_nFeature_RNA.pdf"
...
title = "SDR001_dCas9_UMAP_nReads_gDNA.pdf"
...
title = "SDR001_dCas9_UMAP_nCount_RNA.pdf"
...
title = "SDR001_dCas9_UMAP_nReads_RNA.pdf"
...
title = "SDR001_dCas9_UMAP_sbc_ratio.pdf"
...
out_dir <- here("SDR001_dCas9_Plots/01_Input_cells_txt/")
