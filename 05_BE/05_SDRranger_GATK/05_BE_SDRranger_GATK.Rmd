...
all_var_full <- read_tsv("06_GATK_VEP/BE.vcf", skip = 170, col_names = TRUE)
...
out_put_dir <- "All_variants_plots"
...
#Plot some quality control plots - VAF vs read depth - only for intended ones
...
title = "Volcano_plot_DE_testing_all_genes_POU5F1.pdf"
p <- DE_test_out %>%
  ggplot(aes(x = avg_log2FC, y = -log10(p_val_adj), color = Significant, shape=Test)) +
  scale_color_manual(values = group_colors)+
  #facet_wrap(~target_type, ncol = 4) +
...
ggsave(filename = title, path = here("Plots_GATK/POU5F1_Comb_plots"), plot = p, device = "pdf", scale = 1,
       width = 15, height = 15, units = "cm", dpi = 300)
...
write.csv(DE_test_out, file = here("DE_test_out_POU5F1_comb.csv"), quote = FALSE, row.names = FALSE, col.names = TRUE)
...
ggsave(filename = title, path = here("Plots_GATK/POU5F1_Comb_plots"), plot = pl2, device = "pdf", scale = 1,
...
#Plot the combinations in a dotplot P-value and logFC  - Version 2
...
title = "SDR006_Dot_plot_POU5F1_Combinations_V2.pdf"

pl1 <-  DE_test_out_int %>%
  ggplot(aes(y = ID_Geno1 ,  x = ID_Geno2)) + 
  geom_tile(aes(fill = avg_log2FC)) +  
    scale_fill_gradient2(low = "#990000", mid = "#F6F5F2", high = "#337357", limits = c(-0.7, 0.7)) +  
    	theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1)) +
  	theme(plot.title = element_text(hjust=0.5, face="bold")) +
  	ggtitle(title)
