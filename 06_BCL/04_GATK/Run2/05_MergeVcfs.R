...
annot <- read.delim("/Run2/05_Merged_VCFs/06_BCL_genomic_annotation_file.txt") %>% dplyr::select(Chromosome = seqnames, Start = start, End = end , Gene, ID)
...
write.table(combined_df, file="/Run2/05_Merged_VCFs/06_BCL_Run2_merged.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
write.table(header, file="/Run2/05_Merged_VCFs/header.txt", quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
...
combined_df_filtered <- combined_df_filtered %>% filter(Variant_meta$sum_variant > 8438*0.003)
...
write.table(combined_df_filtered, file="/Run2/05_Merged_VCFs/06_BCL_Run2_merged_cut.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
