...
read_and_preprocess <- function(file_path) {
  read_tsv(file_path, skip = 168, col_names = TRUE, show_col_types = FALSE) %>%
    dplyr::select(-QUAL, -FILTER, -INFO, -FORMAT)
}

# Initialize the combined data frame with the first file's data
combined_df <- read_tsv(skip = 168, files[1], show_col_types = FALSE) %>% mutate(QUAL = ".", INFO = ".")
...
annot <- read.csv("05_BE_genomic_annotation_file.csv") %>% dplyr::select(Chromosome, Start, End , Gene = target_name, ID)
...
write.table(combined_df, file="05_Merged_VCFs/05_BE_merged.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
write.table(header, file="05_Merged_VCFs/header.txt", quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
...
combined_df_filtered <- combined_df_filtered %>% filter(Variant_meta$sum_variant > 5521*0.003)
...
write.table(combined_df_filtered, file="05_BE_merged_cut.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
