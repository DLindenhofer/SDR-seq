...
read_and_preprocess <- function(file_path) {
  read_tsv(file_path, skip = 116, col_names = TRUE, show_col_types = FALSE) %>%
    dplyr::select(-QUAL, -FILTER, -INFO, -FORMAT)
}

# Initialize the combined data frame with the first file's data
combined_df <- read_tsv(skip = 116, files[1], show_col_types = FALSE) %>% mutate(QUAL = ".", INFO = ".")
...
annot <- read.delim("/Run1/05_Merged_VCFs/06_BCL_genomic_annotation_file.txt") %>% dplyr::select(Chromosome = seqnames, Start = start, End = end , Gene, ID)
...
write.table(combined_df, file="/Run1/05_Merged_VCFs/06_BCL_Run1_merged.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
write.table(header, file="/Run1/05_Merged_VCFs/header.txt", quote = FALSE, row.names = FALSE, col.names = FALSE, sep = "\t")
...
combined_df_filtered <- combined_df_filtered %>% filter(Variant_meta$sum_variant > 6359*0.003)
...
write.table(combined_df_filtered, file="/Run1/05_Merged_VCFs/06_BCL_Run1_merged_cut.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
