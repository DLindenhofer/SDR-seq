...
POP_TAP <- read.csv("Input/ipsc_best_inner_primers.csv")


#Load bed files used and combine

cDNA_annotate_1 <- read.delim("Input/ipsc_inner_primers.bed", header = FALSE)
cDNA_annotate_1 <- cDNA_annotate_1[2:31,]




cDNA_annotate <- cDNA_annotate_1 
colnames(cDNA_annotate) <- c("seq_id", "start", "end", "name", "score", "strand", "thickStart", "thickEnd", "itemRgb", "blockCount", "blockSizes", "blockStarts")

#Remove duplicates
cDNA_annotate <- distinct(cDNA_annotate)


POP_TAP <- POP_TAP %>% left_join(cDNA_annotate, by = c("primer_id" = "name"))

write.table(POP_TAP, file="Input/POP_TAP_seq_best_inner_primers_chromosomal_loc_add.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
...
cDNA_targets <- read.delim(file="Input/POP_TAP_seq_best_inner_primers_chromosomal_loc_add.txt")
cDNA_targets <- cDNA_targets %>% dplyr::select(ID = "seq_id.x", Chromosome = "seq_id.y", Start = "start.y", End = "end.y")
...
export(cDNA_targets_coord_gr, "Input/POP_cDNA_targets_coord.bed", "bed")
...
write.table(cDNA_targets_coord, file="Input/POP_cDNA_TAP_REF.tsv", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
...
cDNA_targets <- read.delim("Input/POP_cDNA_TAP_REF.tsv")
...
write.csv(cDNA_targets_seq, file = "Input/POP_cDNA_TAP_REF_seqs.csv", quote = FALSE, row.names = FALSE, col.names = TRUE)

test <- read.csv("Input/POP_cDNA_TAP_REF_seqs.csv")
