...
#Get file names
file_names <- list.files(path = "02_Input_cells_bam", pattern = "\\.bam$", full.names = FALSE)

file_path_add <- "/02_Input_cells_bam/"
...
#Make smaller chunks for submission and submit individual batches as individual scripts (array within script)


#Need to also make smaller chunks as otherwise too many files are handled

out_dir <- "02_Input_GATK/"
