...
annot <- read.delim("05_CRISPRi_gDNA_targets_annotate.txt") %>% dplyr::select(Chromosome, Start, End , Gene, ID = Amp_ID)
...
write.table(combined_df, file="/05_Merged_VCFs/CRISPRi_merged.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
...
combined_df <- read.delim("/05_Merged_VCFs/CRISPRi_merged.txt", header = TRUE)
...
write.table(combined_df_filtered, file="/05_Merged_VCFs/CRISPRi_merged_cut.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
