...
SDR002_TAP <- read.delim("Input/Panel_size_RNA_targets_all.txt")
SDR002_TAP <- cDNA_targets_all_anno
...
cDNA_targets <- SDR002_TAP %>% dplyr::select(ID = "ID", Chromosome = "seqnames", Start = "start", End = "end")
...
export(cDNA_targets_coord_gr, "Input/SDR002_cDNA_TAP_REF_coord.bed", "bed")
...
#REMARK - No overlaps observed

write.table(cDNA_targets_coord, file="Input/SDR002_cDNA_TAP_REF.tsv", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t")
...
cDNA_targets <- cDNA_targets_coord
...
write.csv(cDNA_targets_seq, file = "Input/SDR002_cDNA_TAP_REF_seqs.csv", quote = FALSE, row.names = FALSE, col.names = TRUE)
