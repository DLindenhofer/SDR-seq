...
#Select subset to run pipeline - first 100
target_gene_names <- target_gene_names[1:100]

#Select subset to run pipeline - 101:200
target_gene_names <- target_gene_names[101:200]

#Select subset to run pipeline - 201:307
target_gene_names <- target_gene_names[201:307]
...
#Input bam file from previous data from Huber group
scRNAseq_bam <- "TAP_seq_BCL_Cite-seq_Donnacha/possorted_genome_bam.bam"
...
export(polyA_sites, con = "./BCL_polyA_sites.bed", format = "bed")
export(unlist(target_genes), con = "./BCL_target_genes.gtf", format = "gtf")
...
inner_primers_df_100 <- inner_primers_df
inner_primers_df_200 <- inner_primers_df
inner_primers_df_300 <- inner_primers_df

write.csv(inner_primers_df,'SDR005_BCL_best_inner_primers.csv')
...
exportPrimerTrack(inner_primers_track, con = "./BCL_inner_primers.bed")
...
#Do if necessary to cross check primers with MB ones
...
MB_primers_check <- read.csv(file = "MB_primers_for_checking_comp.csv")
...
ggsave(filename = "inner_primer_MB_primer_comp_predictions.pdf", path = plot_save_dir, plot = p, device = "pdf", scale = 1,
